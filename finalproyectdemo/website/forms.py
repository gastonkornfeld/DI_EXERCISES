...
class AddYogaPost(FlaskForm):
    ...
    secondary_content = TextAreaField('Here the secondary content (contenido secundario)',
                        validators=[DataRequired(), Length(min=2, max=200000)])
...
class AddRecipe(FlaskForm):
    ...
    title = StringField('Set the title of the recipie, (Titulo de la receta)',
                        validators=[DataRequired(), Length(min=2, max=1000)])
    ingredients = TextAreaField('Here to put all the ingredients of the recipie (Seccion ingredientes)',
                        validators=[DataRequired(), Length(min=2, max=200000)])
    preparation = TextAreaField('Here the preparation of the recipie (Seccion preparacion de la receta)',
                        validators=[DataRequired(), Length(min=2, max=200000)])
    extra = TextAreaField('Any extra information needed to post  (Informacion complementaria)',
...
class LeaveYourEmailForm(FlaskForm):
    ...
    """
    email = StringField('Leave your email to receive our newsletter',
                        validators=[DataRequired(), Email()])
...
class AddCourse(FlaskForm):
    ...
    description = TextAreaField('ADD A DESCRIPTION FOR THE COURSE (Descripcion del curso)',
                        validators=[DataRequired(), Length(min=2, max=20000)])
