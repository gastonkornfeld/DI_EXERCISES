...
if (wordnot < wordbad && wordnot != -1) {
    // with this reggex we sustitute the inside words in "not"..."bad", for good
    // then i stored in another variable

    sentence2 = sentence.replace(/(not)(.+?)(?= bad)/, "$1 good");

    // then i will take the word bad and not from the sentence
    // and log in to the console the correct one on each case
