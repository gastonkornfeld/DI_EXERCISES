...
def add_user():
    ...
    if form.validate_on_submit():
        ...
        status = 'client'
        new_user = models.User(name = name, street = street, city = city, zip_code = zipcode, status = status)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if models.User.query.filter_by(name=name, city = city).first() == None:
            ...
            user_logged = models.User.query.filter_by(name=name, city = city).first()
            if user_logged.status == "Admin":
                flask.flash(f'Login as administrator Succesfully', 'success')
                return flask.render_template('users.html', userlog = user_logged, all_users = all_users)
            else:
                flask.flash(f'Login as client Succesfully', 'success')
                return flask.render_template('users.html', userlog = user_logged, all_users = all_users)
